...
#
#  PURPOSE: Python code implementing the proposed method.
#
#===============================================================================
...
def fit_dnn(formula, 
    ...
    """
    Fit the Neural Expectation-Maximization Semi-Competing Risks (NEM-SCR) model

    Implements the full NEM-SCR algorithm described in the manuscript, with:

      - **E-step**: Closed-form updates of individual frailties 
      - **M-step**: Nonparametric baseline hazard updates for each transition  
      - **N-step**: Stochastic gradient descent updates for transition-specific
                    neural networks (risk functions h_g and frailty variance)

    The method estiamtes three transition hazards in a semi-competing risks
    (illness-death) model:

      1. Non-terminal  
      2. Terminal without prior non-terminal event  
      3. Terminal after non-terminal event

    A shared Gamma frailty induces dependence across transitions. The neural
    networks learn nonparametric log-risk functions, while the baseline hazards
    remain fully nonparametric (piecewise constant jumps at event times).

    Parameters
    ----------
    form : dict
        A dictionary specifying outcome and predictor columns, e.g.:
        ...
        All listed columns must appear in `data`.

    data : pandas.DataFrame
        A dataset containing:
            - Y1, D1: time and event indicator for the non-terminal composite
            - Y2, D2: time and event indicator for the terminal event
            - X predictors (used for all 3 transitions)
            - h1, h2, h3 (optional true risks for simulation monitoring)

    na_action : {"na.fail", "na.omit"}, default = "na.fail"
        Behavior for rows containing missing values.

    dim_layers : list of int
        Widths of the three hidden layers for each neural network. The same
        architecture is used for all three transition-specific subnetworks.
    ...
    max_epochs_theta : int
        Maximum number of inner SGD updates for the frailty variance
        during each N-step.

    max_epochs_n : int
        Maximum number of neural network minibatch passes per EM iteration.

    verbose : bool
        Whether to print epoch-level summaries and (optionally) plot 
        intermediate baseline hazards.

    tol : float
        Convergence tolerance for EM updates of the baseline hazards.

    theta0 : float
        Initial value for the frailty variance parameter.

    lr_theta : float
        Learning rate for fraily variance during the N-step SGD updates.

    batch_size : int
        Minibatch size for neural network training.

    Returns
    -------
    dict with keys:
        "theta" : float
            Final estimate of the frailty variance parameter.
        "lam01", "lam02", "lam03" : numpy.ndarray
            Estimated nonparametric baseline hazards for transitions 1-3.
        "h1", "h2", "h3" : torch.Tensor
            Final fitted log-risk functions evaluated at observed X.
        "gamma" : numpy.ndarray
            Posterior frailty means E[gamma_i | data].

    Notes
    -----
    - The function assumes a Gamma(1/theta, 1/theta) frailty distribution.
    - All three subnetworks share the same architecture but have separate
    - parameters, enabling transition-specific nonlinear effects.
    - Baseline hazards are fully nonparametric.
    - The estimation procedure alternates between analytic updates
      (E/M-steps) and neural SGD (N-step) until convergence or until the
